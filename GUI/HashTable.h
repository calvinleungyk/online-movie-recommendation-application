...
private:
      ...
      std::vector< HashItem<keyType, valueType> > storage;
public:
  ...
  class Iterator 
  {
    private:
      ...
      Iterator (const HashTable<keyType, valueType> *m, std::vector< HashItem <keyType, valueType> > s, int ct)
      { 
        storage = s;
        count = ct;
        whoIBelongTo = m;
    ...
    public:
      ...
      {
        return ( (curr == other.curr) && (whoIBelongTo == other.whoIBelongTo) && (storage == other.storage));
      }
    ...
    {
      return HashTable<keyType, valueType>::Iterator(this, storage, 0);
    };
       // returns an iterator initialized to the first element

     HashTable<keyType,valueType>::Iterator end () const
     {
      return HashTable<keyType, valueType>::Iterator(this, storage, (storage.size()-1));
     };
    ...
    int returnCount()
     {
      ...
      void insert(keyType key, valueType value) {
        HashItem<keyType, valueType> h (key, value);
        storage.push_back(h);
        int hash = 0;
        std::stringstream ss;
        int key_int;
